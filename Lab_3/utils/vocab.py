...
class Vocab(object):
    ...
    def make_vocab(self, config):
            ...
            try:
                ...
                with open(path, "r", encoding="utf-8") as f:
                    ...
                    if isinstance(data, list):  # Nếu file là một danh sách các đối tượng JSON
                        for item in data:
                            self.process_item(item, config, words_counter, labels)
                    else:
                ...
                with open(path, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if line:  # Bỏ qua các dòng trống
                            ...
                            self.process_item(item, config, words_counter, labels)
        # Take most common words up to vocab size
        num_words = len(words_counter.keys())
        self.vocab_size = num_words + len(self.specials) 

        most_common_words = words_counter.most_common(num_words)
        ...
        for idx, (word, _) in enumerate(most_common_words, start=len(self.specials)):
            self.stoi[word] = idx
            self.itos[idx] = word
    ...
    def decode_sentence(self, input_ids: torch.Tensor, join_words=True) -> List[str]:
        ...
        return sentences
    def encode_sequence_labeling(self, text: str, max_len: int = None) -> torch.Tensor:
    ...
    def get_pad_idx(self) -> int:
        ...
        return self.pad_id
    def process_item(self, item, config, words_counter, labels):
        """
